...
class DatabaseManager {
  ...
  setAllCustomCatalogSortMode(sortMode = 'rss_date_desc') {
    const rows = this.db.prepare('SELECT id, filters FROM custom_catalogs').all();
    const update = this.db.prepare(
      'UPDATE custom_catalogs SET filters = ?, updated_at = ? WHERE id = ?'
    );
    let changed = 0;
    const now = Date.now();
    const apply = this.db.transaction(() => {
      for (const row of rows) {
        ...
        if (filters.sort_mode === sortMode) continue;
        update.run(JSON.stringify({ ...filters, sort_mode: sortMode }), now, row.id);
        changed++;
      }
    });
    apply();
