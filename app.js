...
const CATALOG_SORT_OPTIONS = [
  ['rss_date_desc', 'catalogs_sort_rss_desc'],
  ['rss_date_asc', 'catalogs_sort_rss_asc'],
  ['added_desc', 'catalogs_sort_added_desc'],
  ['added_asc', 'catalogs_sort_added_asc'],
  ['year_desc', 'catalogs_sort_year_desc'],
  ['year_asc', 'catalogs_sort_year_asc'],
  ['name_asc', 'catalogs_sort_name_asc'],
  ['name_desc', 'catalogs_sort_name_desc']
];

function catalogSortOptions(selected) {
  return CATALOG_SORT_OPTIONS.map(([value, labelKey]) =>
    `<option value="${value}"${value === selected ? ' selected' : ''}>${escHtml(t(labelKey))}</option>`
  ).join('');
}
  ...
  container.innerHTML = catalogManagerData.catalogs.map(catalog => {
    ...
    return `<div class="manager-row">
      <div class="manager-row-main">
        ...
        <div class="manager-row-meta">${escHtml(typeLabel)} · ${escHtml(years)} · ${catalog.source_urls.length ? `${catalog.source_urls.length} ${t('catalogs_source_count')}` : t('catalogs_all_sources')} · ${escHtml(sortLabels[sortMode] || sortLabels.rss_date_desc)}${guide ? ` · ${t('catalogs_guide_prefix')} ${escHtml(guide.name)}` : ''}</div>
        <div class="manager-row-meta">
          ${catalog.updates_enabled ? `↻ ${t('catalogs_updates_active')}` : `⏸ ${t('catalogs_frozen')}${catalog.frozen_at ? ` ${new Date(catalog.frozen_at).toLocaleString()}` : ''}`}
          · ${catalog.enabled ? t('catalogs_visible') : t('catalogs_hidden')}
        </div>
      </div>
      <div class="manager-row-actions">
        <button class="btn-sm" onclick="editCatalog('${catalog.id}')">${t('catalogs_edit')}</button>
        <button class="btn-sm" onclick="toggleCatalogUpdates('${catalog.id}', ${!catalog.updates_enabled})">${catalog.updates_enabled ? t('catalogs_pause_updates') : t('catalogs_resume_updates')}</button>
        <button class="btn-sm" onclick="toggleCatalogExposure('${catalog.id}', ${!catalog.enabled})">${catalog.enabled ? t('catalogs_hidden') : t('catalogs_visible')}</button>
        <button class="btn-danger btn-sm" onclick="deleteCatalog('${catalog.id}')">${t('sources_delete')}</button>
      </div>
      <div class="catalog-inline-sort">
        <label>${escHtml(t('catalogs_sort'))}</label>
        <select onchange="this.dataset.changed='true'">${catalogSortOptions(sortMode)}</select>
        <button class="btn-sm" onclick="saveCatalogSort('${catalog.id}', this.previousElementSibling)">${t('catalogs_sort_save')}</button>
      </div>
    </div>`;
  }).join('');
}
...
async function saveCatalog() {
  ...
  if (!name) {
    ...
    return alert(t('catalogs_name_required'));
  }
...
async function saveCatalogSort(id, select) {
  ...
  try {
    const response = await fetch(`/api/catalogs/${id}`, {
      ...
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        filters: { ...(catalog.filters || {}), sort_mode: select.value }
      })
    });
    const data = await response.json();
    if (!response.ok) return alert(data.error || t('catalogs_save_error'));
    await loadCatalogManager();
  } finally {
    if (button) button.disabled = false;
  }
}
window.saveCatalogSort = saveCatalogSort;
